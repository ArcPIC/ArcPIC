        ...
        if ( n_ion < n_n ) {
          ...
          for (size_t i_ion = 0; i_ion < n_ion; i_ion++) {
            ...
            if ( RAND < n_n*W_0*S_i*dti/(2*jr+1) ) { // correct Si with 2D volume factor (2j+1) here
              *engcheck += 0.5*ions->mass*(SQU(ions->vz[i_ion + Next_ion]) +
                                           SQU(ions->vr[i_ion + Next_ion]) +
                                           SQU(ions->vt[i_ion + Next_ion]));
              *engcheck += 0.5*neutrals->mass*(SQU(neutrals->vz[i_n + Next_n]) +
                                               SQU(neutrals->vr[i_n + Next_n]) +
                                               SQU(neutrals->vt[i_n + Next_n]));
      ...
      if ( n_m && n_el ) {
        ...
        for (size_t i_el=0; i_el<n_el; i_el++) {
          ...
          if (W2_0 >= E_th && RAND < nm_real*W_0*S_i/(2*jr+1) ) { // correct Si with 2D volume factor (2j+1) here
            ...
            /*
             *engcheck += 0.5*m_e*(SQU(electrons[i_el + Next_el].p.vz) +
                                   SQU(electrons[i_el + Next_el].p.vr) +
                                   SQU(electrons[i_el + Next_el].p.vt));
             *engcheck += 0.5*molecules->mass*(SQU(molecules[i_m + Next_m].p.vz*dti) +
                                               SQU(molecules[i_m + Next_m].p.vr*dti) +
                                               SQU(molecules[i_m + Next_m].p.vt*dti))
                        - 0.5*m_e*molecules->mass/(m_e+molecules->mass)*E_th ;
            ...
            */
             ...
             *engcheck -= 0.5*m_e*(SQU(electrons[i_el + Next_el].p.vz) +
                                   SQU(electrons[i_el + Next_el].p.vr) +
                                   SQU(electrons[i_el + Next_el].p.vt));
             *engcheck -= 0.5*molecules->mass*(SQU(target_v.z) +
                                               SQU(target_v.r) +
                                               SQU(target_v.t));
        ...
        if ( n_el<Nn_real ) {
          ...
          for (size_t i_el=0; i_el<n_el; i_el++ ) {
            ...
            if (W2_0 >= E_th && RAND < Nn_real*W_0*S_i/(2*jr+1) )  {// correct Si with 2D volume factor (2j+1) here
              ...
              *engcheck += 0.5*m_e*(SQU(electrons->vz[i_el + Next_el]) +
                                    SQU(electrons->vr[i_el + Next_el]) +
                                    SQU(electrons->vt[i_el + Next_el]));
              *engcheck += 0.5*neutrals->mass*(SQU(neutrals->vz[i_n + Next_n]*dti) +
                                               SQU(neutrals->vr[i_n + Next_n]*dti) +
                                               SQU(neutrals->vt[i_n + Next_n]*dti))
                         - 0.5*m_e*neutrals->mass/(m_e+neutrals->mass)*E_th;
              ...
              *engcheck -= 0.5*(neutrals->mass-m_e)*(SQU(ions->vz.back()*dti) +
                                                     SQU(ions->vr.back()*dti) +
                                                     SQU(ions->vt.back()*dti));
              ...
              *engcheck -= 0.5*m_e*(SQU(electrons->vz[i_el + Next_el]) +
                                    SQU(electrons->vr[i_el + Next_el]) +
                                    SQU(electrons->vt[i_el + Next_el]));
              *engcheck -= 0.5*m_e*(SQU(electrons->vz.back()) +
                                    SQU(electrons->vr.back()) +
                                    SQU(electrons->vt.back()));
